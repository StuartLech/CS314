...
ProcessModuleInit ()
{
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    ...
    //Initinitialize number of tickets each process has
    //
    //
    //
    pcbs[i].numTickets = 100;
    //
    //
    //
  }
...
ProcessSchedule ()
{
  ...
  int           winningTicket;
  //------------------------------------------------------------
  //  store the total amount of tickets on the run queue
  int		totalTickets;
  totalTickets = 0;
  currTicket = 0;
  //-------------------------------------------------------------
  ...
  //--------------------------------
  //   calulating total amount of tickets on the run queue
  //
  //-----------------------------------------------------
  currL = QueueFirst(&runQueue);
  for (i = 0; i < QueueLength (&runQueue); i++) {
	  currPCB = (PCB*)(currL->object);
          totalTickets += currPCB->numTickets;
	  currL = currL->next;
  }
  dbprintf('z',"Total ticket in ProcessSchedule: %d\n", totalTickets);
  winningTicket = random() % totalTickets + 1;
  dbprintf('z', "Winning tikcet is %d\n", winningTicket);
  currL = QueueFirst(&runQueue);
  currPCB = (PCB*)(currL->object);
  for (i = 0; i < winningTicket; i++) {
          if (currTicket == (currPCB->numTickets)) {
			  currL = currL->next;
			  currPCB = (PCB*)(currL->object);
			  currTicket = 0;
  ...
  // Change between round robin and lottery scheduling here by commenting out each of the line below
 // pcb = (PCB *)((QueueFirst (&runQueue))->object); // comment here for lottery schediling
  pcb = currPCB;				   // comment here for round robin
...
ProcessFork (VoidFunc func, uint32 param, int p_nice, int p_info,char *name, int isUser)
{
  ...
  QueueRemove (l);
  pcb = (PCB *)(l->object);
  //each p_nice value represent multiple tickets
  //
  //
  pcb->numTickets += p_nice * 100;
  //
  //
  dbprintf('k',"Number of current pcb tickets is %d\n", pcb->numTickets);
